...
class RequestApi(object):
    def __init__(self, appid, secret_key, upload_file_path, file_piece_sice, api_prepare,\
                 api_upload, api_merge, api_get_progress, api_get_result,lfasr_host):
        self.appid = appid
        self.secret_key = secret_key
        self.upload_file_path = upload_file_path
        self.file_piece_sice = file_piece_sice
        self.api_prepare = api_prepare
        self.api_upload = api_upload
        self.api_merge = api_merge
        self.api_get_progress = api_get_progress
    ...
    def gene_params(self, apiname, taskid=None, slice_id=None):
        ...
        api_prepare = self.api_prepare
        api_upload = self.api_upload
        api_merge = self.api_merge
        api_get_progress = self.api_get_progress
        api_get_result = self.api_get_result
        ...
        if apiname == api_prepare:
            file_piece_sice = self.file_piece_sice
    ...
    def gene_request(self, apiname, data, files=None, headers=None):
        ...
        if result["ok"] == 0:
            ...
            with open('result.txt', 'w', encoding='utf-8') as f:
                if apiname != '/prepare' and apiname != '/upload' and apiname != '/merge' and apiname != '/getProgress':
                    #print(result['data'])
                    data0 = eval(result['data'])
                    for i in data0:
                        f.write(i['onebest'])
                        f.write("\n")
            return result
    ...
    # 预处理
    def prepare_request(self,api_prepare):
    ...
    def all_api_request(self, appid, secret_key, upload_file_path, file_piece_sice, api_prepare,\
                 api_upload, api_merge, api_get_progress, api_get_result):
        # 1. 预处理
        pre_result = self.prepare_request(api_prepare)
...
if __name__ == '__main__':
    ...
    my_audio_clip.write_audiofile("test.wav")
    ...
    appid = "96d1b2e0"
    secret_key = "0220b2e2148e0558ac73c71792eb72a1"
    upload_file_path = r"test.wav"

    # ——————————————————转写可配置参数————————————————
    # 参数可在官网界面（https://doc.xfyun.cn/rest_api/%E8%AF%AD%E9%9F%B3%E8%BD%AC%E5%86%99.html）查看，根据需求可自行在gene_params方法里添加修改
    api = RequestApi(appid="96d1b2e0", secret_key="0220b2e2148e0558ac73c71792eb72a1", upload_file_path=r"test.wav",\
                    file_piece_sice = 10485760,\
    ...
    result_file = open("final_result.txt", 'w', encoding='utf-8')
